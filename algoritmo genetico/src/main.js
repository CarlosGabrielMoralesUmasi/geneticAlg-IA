...
$(function () {
  ...
  $("#start_btn").click(function () {
    if (points.length >= 3) {
      ...
      alert("añade algunos puntos más al mapa, oe gei");
    }
  });
...
function initData() {
  ...
  currentGeneration = 0;
  currentBest;
  population = []; //nueva matriz (Poblacion_SIZE);
  ...
  if (running) {
    ...
    $("#status").text(
      "HAY " +
        ...
        "th generacion con " +
        mutationTimes +
        " tiempos de mutacion(itera). mejor valor: " +
        ~~bestValue
    );
  } else {
    $("#status").text("hay " + points.length + " ciudades en el mapa. ");
  }
  clearCanvas();
  if (points.length > 0) {
